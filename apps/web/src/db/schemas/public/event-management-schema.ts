/**
 * Public schema – event_logs (Event management domain).
 */
...
/** RLS: authenticated user can only read/insert rows where user_id = auth.uid(). */
...
export const eventLogs = pgTable(
  ...
  table => ({
    ...
    selectPolicy: pgPolicy('Enable read for own user', {
      for: 'select',
      to: authenticatedRole,
      using: sql`${table.user_id} = (SELECT auth.uid())`,
    }),
    insertPolicy: pgPolicy('Enable insert for own user', {
      for: 'insert',
      to: authenticatedRole,
