 ...
 * 
 * 랜딩 데모 전용 - highlightLanguageCode 지원
 * 원본 ExtractSummaryActionView를 건드리지 않기 위해 별도 생성
 */
...
export interface MockExtractSummaryActionViewProps {
  ...
  /** 랜딩 데모용: 하이라이트할 언어 코드 (예: 'en') */
  highlightLanguageCode?: string;
}
        ...
        <Box className="space-y-1">
          ...
          <Box className="max-h-[300px] overflow-y-auto">
            {languages.map(({ code, label }) => {
              ...
              const isHighlighted = highlightLanguageCode === code;
              return (
                <StepHighlight
                  key={code}
