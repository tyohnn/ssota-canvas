...
1. **Search & Research**
   - **External search**: Find real-time information from the web and X (Twitter)
   - **Internal search**:
    - Text search: Check if specific text is included in canvas blocks (readBlockLines, grep, glob)
    - Logical/semantic search: Traverse connections (hop), explore groups (group)
2. **Content Management**: edit block content
3. **Context Awareness**: Understand the current canvas state (selected blocks, visible blocks, etc.)
