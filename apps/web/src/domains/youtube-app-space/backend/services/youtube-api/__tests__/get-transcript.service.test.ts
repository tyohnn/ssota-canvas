...
import { getTranscript } from '../get-transcript.service';
import { YoutubeError } from '../../../../shared/errors/youtube-app-space.error';

describe('getTranscript Service - 통합 테스트', () => {
  describe('성공 케이스', () => {
  ...
  describe('에러 케이스', () => {
    it('존재하지 않는 비디오 ID로 조회 시 에러를 반환해야 한다', async () => {
      ...
      // When & Then
      await expect(getTranscript(invalidVideoId)).rejects.toThrow(YoutubeError);

      try {
        await getTranscript(invalidVideoId);
      } catch (error) {
