 ...
 * 1. 요약 처리 서비스 호출 (비즈니스 로직 오케스트레이션)
...
import {
  type ProcessVideoSummaryRequest,
...
/**
 * Video 요약 처리 Action (blockId 기반, 언어별)
 */
export const processVideoSummaryAction = withYoutubeBlockSecureAction(
  ExtractVideoSummaryRequestSchema,
  'processVideoSummaryAction',
  processVideoSummaryInternal,
  {
    getLogMetadata: req => ({
      blockId: req.blockId,
 ...
 */
async function processVideoSummaryInternal(
  safeDto: ProcessVideoSummaryRequest,
  ...
  try {
    ...
    console.error('[processVideoSummaryInternal] Internal error:', error);
