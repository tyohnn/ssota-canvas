...
export function useNodeOperations() {
  ...
  const createBlockWithValidation = useCallback(
  ...
  const updateNodeWithValidation = useCallback(
    async (input: UpdateNodeInput) => {
      ...
      try {
        ...
        const existingNode = await getBlock(input.id);
  ...
  const validateNodeData = useCallback(
    async (input: Omit<CreateBlockInput, "position">) => {
