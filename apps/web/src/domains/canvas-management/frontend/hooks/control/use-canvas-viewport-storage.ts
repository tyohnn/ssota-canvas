...
export interface UseCanvasViewportStorageResult {
  // 뷰포트 상태 저장/로드
  getViewportState: (pageId: string) => ViewportState | null;
  setViewportState: (
    pageId: string,
  ...
  // 정리 함수
  clearCanvasStorageForPage: (pageId: string) => void;
}
...
/**
 * Canvas Viewport Storage Hook
 *
 * 로컬 스토리지를 DB로 바라보는 비즈니스 로직 훅
 * - 뷰포트 상태 저장/로드 (페이지별)
 * - 선택된 블럭 저장/로드
 * - 스냅 설정 저장/로드
 */
export function useCanvasViewportStorage(): UseCanvasViewportStorageResult {
  // 뷰포트 상태 저장/로드 함수들
  const getViewportState = useCallback(
    (pageId: string): ViewportState | null => {
      try {
  ...
  // 선택된 블럭 저장/로드 함수들
  const getSelectedBlocks = useCallback((): string[] => {
    try {
      if (typeof window === 'undefined') return [];
  ...
  const setSelectedBlocks = useCallback((blockIds: string[]): void => {
    try {
      if (typeof window === 'undefined') return;
  ...
  // 스냅 설정 저장/로드 함수들
  const getSnapSettings = useCallback((): SnapSettings => {
    try {
  ...
  const setSnapSettings = useCallback((settings: SnapSettings): void => {
    try {
      if (typeof window === 'undefined') return;
  ...
  const clearCanvasStorageForPage = useCallback((pageId: string): void => {
    try {
      if (typeof window === 'undefined') return;
