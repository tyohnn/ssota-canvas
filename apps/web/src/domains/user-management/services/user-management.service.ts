...
export class UserManagementService {
  ...
  async createUserProfile(
    ...
    try {
      ...
      if (!supabaseUser || supabaseUser.id !== command.userId) {
        return Result.error(
          new UserManagementError('USER_NOT_FOUND', 'User not found in Supabase Auth')
        );
      }
      ...
      return Result.error(
        new UserManagementError('PROFILE_CREATION_FAILED', 'Failed to create user profile', {
          error,
        })
      );
    }
  ...
  private async createDefaultOrganizationInternal(user: UserAggregate): Promise<void> {
