  ...
  describe('save', () => {
    ...
    it('중복 ID는 거부해야 한다', async () => {
      ...
      vi.spyOn(errorRepository, 'save').mockRejectedValue(
        new UserManagementError('ORGANIZATION_CREATION_FAILED', 'Organization with this ID already exists')
      );

      // When & Then
      await expect(errorRepository.save(mockOrganization)).rejects.toThrow(UserManagementError);
