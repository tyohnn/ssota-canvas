...
import type { SourceSummaryRuntimeDeps } from '@/domains/source-management/frontend/adapters/contracts/runtime-deps';
...
export function useSourceSummarySection(
  ...
  // 2. Job 완료 시 tabOptions.isExtracting 해제 (AI status 패널 "Generating" 종료) — caller provides onJobCompleted
  const onJobCompletedCb = useCallback(() => {
    onJobCompleted();
  }, [onJobCompleted]);
  ...
  const {
    ...
    workspaceId,
    readonly,
    onJobCompleted: onJobCompletedCb,
  });

  // 4. 블록 액션 버튼 등에서 Summary 탭 진입 시 지정된 언어 — from deps
  const initialTabLanguage = getInitialTabLanguage(blockSlug);
  ...
  const extractMutation = useSummaryExtractMutation(
    ...
    { workspaceId, setAutoSummaryBlockId: deps.setAutoSummaryBlockId }
  );
  ...
  const isExtractingFromTabOptions = getIsExtractingFromTabOptions(blockSlug);
