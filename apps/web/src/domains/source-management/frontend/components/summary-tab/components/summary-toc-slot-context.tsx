...
const SummaryTOCSlotContext = createContext<SummaryTOCSlotContextValue | null>(
  null
);
...
export function SummaryTOCSlotProvider({ children }: { children: ReactNode }) {
  ...
  const setTOCData = useCallback(
    (newData: { tiptapContent: unknown; showTOC: boolean } | null) => {
      setData((prev) => {
        if (newData === null) return null;
        if (
          prev &&
          prev.tiptapContent === newData.tiptapContent &&
          prev.showTOC === newData.showTOC
        )
          return prev;
        return newData;
      });
  ...
  return (
    <SummaryTOCSlotContext.Provider value={value}>
      {children}
    </SummaryTOCSlotContext.Provider>
  );
}
