...
import { useReactFlowCanvasAdapter, type ReactFlowCanvasState } from "@/domains/canvas/adapters/useReactFlowCanvasAdapter";
...
export function IntegratedReactFlowCanvas() {
  ...
  const [currentBlocksWithPositions, setCurrentBlocksWithPositions] = useState<BlockWithPosition[]>([]);
  ...
  }, [canvasData.canvasMode, canvasData.selectedPageId, canvasData.selectedComponentId]);
  
  // 페이지가 바뀔 때마다 데이터 로딩
  useEffect(() => {
    if (contextId && canvasData.canvasMode === "page") {
      pageCommands.loadPageData(contextId)
        .then((result) => {
      ...
      // 페이지가 선택되지 않았거나 컴포넌트 모드인 경우 초기화
  ...
  const [reactFlowState, setReactFlowState] = useState<ReactFlowCanvasState>({ nodes: [], edges: [] });

  // 데이터가 변경될 때마다 React Flow 상태 업데이트
  useEffect(() => {
    const newState = getReactFlowState(currentBlocksWithPositions, currentEdges, canvasData);
