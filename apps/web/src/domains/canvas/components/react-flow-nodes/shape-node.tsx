...
export function ShapeNode({
  ...
  const setWeight = useCallback(
    async (nextWeight: NonNullable<ShapeNodeData["weight"]>) => {
      // Update block metadata via commands
      const block = getBlockById(id);
      if (block) {
        const metadata = (block.metadata || {}) as any;
      ...
      <div className="pointer-events-none absolute inset-0 grid place-items-center">
        {selected ? (
          <textarea
            ...
            onInput={autoResize}
            className="pointer-events-auto h-auto min-h-[1.4em] w-[calc(100%-12px)] resize-none overflow-hidden rounded bg-transparent px-1 py-0.5 leading-tight text-center outline-none placeholder:text-foreground/40 focus:ring-0 whitespace-pre-wrap break-words"
            style={{
              fontSize: fontSize,
              fontWeight: weight,
          ...
          <div
            className="pointer-events-none h-auto min-h-[1.4em] w-[calc(100%-12px)] px-1 py-0.5 leading-tight text-center whitespace-pre-wrap break-words"
            style={{
              fontSize: fontSize,
              fontWeight: weight,
              color: ShapePolicy.getTextColor(color),
