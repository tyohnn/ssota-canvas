...
export class NodeManagementLogic {
  ...
  static async validateNodeData(
    ...
    try {
      ...
      const existingNodes = await getWorkspaceNodes(input.workspaceId);
      const slugExists = existingNodes.data?.some(
        (node: any) => node.slug === input.slug
      );
      if (slugExists) {
   ...
   */
  static async deleteNodeWithRelationships(nodeId: string) {
    try {
      // Get node edges
      const sourceEdges = await getEdgesBySource(nodeId);
      const targetEdges = await getEdgesByTarget(nodeId);
  ...
  static async validateNodeConnection(
    ...
    if (!targetNode.success || !targetNode.data) {
    ...
    const sourceEdges = await getEdgesBySource(sourceNodeId);
    const existingConnection = sourceEdges.data?.find(
      (edge: any) =>
        edge.target_node_id === targetNodeId && edge.edge_type === edgeType
    );
   ...
   * Get node hierarchy
   */
  static async getNodeHierarchy(nodeId: string) {
