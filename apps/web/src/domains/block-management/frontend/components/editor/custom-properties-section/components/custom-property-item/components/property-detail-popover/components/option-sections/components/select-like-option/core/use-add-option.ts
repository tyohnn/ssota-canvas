...
export function useAddOption() {
  const { options, handleCreateOption } = useOptionManagementContext();
  ...
  // Plus 버튼 클릭 시 즉시 새 옵션 생성 및 DB 저장 후 팝오버 열기
  const handleAddOption = useCallback(async () => {
    try {
      // 즉시 DB에 저장 (optimistic update)
      const newOption = await handleCreateOption('새 옵션', ColorToken.GRAY);
