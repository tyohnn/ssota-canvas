  ...
  const editor = useEditor({
    ...
    onUpdate: ({ editor }) => {
      ...
      if (currentContent === previousContentRef.current) {
        console.log(
          '[BlockContentSection] onUpdate triggered but content unchanged, skipping save'
        );
        return; // 콘텐츠 변경 없음 → 저장 안 함
      }

      // 콘텐츠가 실제로 변경됨
      console.log('[BlockContentSection] Content changed, saving...');
      ...
      try {
        ...
        if (editor) {
          try {
            ...
            console.log(
              '[BlockContentSection] Extracted markdown:',
              contentRaw.slice(0, 100)
            );
          } catch (error) {
            console.warn(
              '[BlockContentSection] Failed to extract markdown:',
              error
            );
          }
        }
