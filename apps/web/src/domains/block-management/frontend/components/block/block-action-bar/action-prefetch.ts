/**
 * Block Action Prefetch Utility
 *
 * 블록 타입별 action 컴포넌트를 미리 로드하여 lagging 방지
 *
 * 전략:
...
function getComponentName(blockType: string): string {
  ...
  return `${capitalize(blockType)}ActionItems`;
}

/**
 * 블록 타입의 action을 prefetch하고 컴포넌트를 레지스트리에 저장
 *
 * @param blockType - 블록 타입
 * @returns Promise<void>
 */
export async function prefetchAction(blockType: string): Promise<void> {
  // Action Items가 없는 블록은 스킵
  ...
  const promise = import(
    ...
    .then(module => {
      ...
      if (Component) {
        ...
        console.error(
          `Component ${componentName} not found in ${blockType}/action-items`
        );
    ...
    .catch(err => {
      console.warn(`Failed to prefetch action for ${blockType}:`, err);
      prefetchCache.set(blockType as BlockType, 'error');
      prefetchPromises.delete(blockType as BlockType);
    });
...
/**
 * 여러 블록 타입의 action을 동시에 prefetch
 *
 * @param blockTypes - 블록 타입 배열
 * @returns Promise<void>
 */
export async function prefetchActions(blockTypes: string[]): Promise<void> {
  const promises = blockTypes.map(type => prefetchAction(type));
 ...
 * @returns boolean
 */
export function isActionLoaded(blockType: string): boolean {
