 ...
 *
 * 엔지니어가 배선하는 비즈니스 로직. EditorPanelBusinessLogic은 @workspace/editor-panel에서 정의.
 */
...
export function useEditorPanelBusiness(
  ...
  return { onTitleSave, onClose } as EditorPanelBusinessLogic;
}

/**
 * Standalone 비즈니스 로직 (Drive 등 React Flow 없는 컨텍스트)
 * - React Flow, useCanvasMetadata 미사용
 * - workspaceId를 파라미터로 받아 서버 액션만 호출
 */
export function useStandaloneEditorPanelBusiness(
  onClose: () => void,
  workspaceId: string
): EditorPanelBusinessLogic {
  const onTitleSave = useCallback(
