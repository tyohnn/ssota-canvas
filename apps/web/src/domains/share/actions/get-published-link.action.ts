...
import { getPublishedLink } from '../backend/services';
import {
  GetPublishedLinkRequest,
  GetPublishedLinkRequestSchema,
} from '../shared/dtos/request';
import { PublishedLinkViewDTO } from '../shared/dtos/response';
 ...
 */
export const getPublishedLinkAction = withSharePageSecureAction(
  GetPublishedLinkRequestSchema,
  'getPublishedLinkAction',
  getPublishedLinkInternal,
  {
    getLogMetadata: req => ({
      pageId: req.pageId,
    }),
 ...
 */
async function getPublishedLinkInternal(
  safeDto: GetPublishedLinkRequest,
  context: PageActionContext
): Promise<ActionResult<PublishedLinkViewDTO | null>> {
  try {
    // 1. Repository 생성
