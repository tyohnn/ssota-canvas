...
export function useReactFlowStyleCommands() {
   ...
   */
  const updateColorById = useCallback(
    async (
      nodeId: string,
      color: AllNodeUIProperties['color']
    ): Promise<CreateStatus> => {
      ...
      try {
        ...
        const dbPayload = {
          ...
          metadata: {
            ...
            formSchema: extractUserDefinedSchema(
              node.type as BlockType,
              updatedData.formSchema
            ),
        ...
        if (isFailure(dbResult)) {
          ...
          return {
            ok: false,
            error: dbResult.error || 'Failed to update size in database',
          };
        }
        ...
        return {
          ok: false,
          error: `Failed to update size: ${error instanceof Error ? error.message : String(error)}`,
        };
      }
    },
    [updateNode]
   ...
   */
  const updateSizeById = useCallback(
    async (
      nodeId: string,
      size: Partial<AllNodeUIProperties['size']>
    ): Promise<CreateStatus> => {
   ...
   */
  const updateFontSizeById = useCallback(
    async (
      nodeId: string,
      fontSize: AllNodeUIProperties['fontSize']
    ): Promise<CreateStatus> => {
   ...
   */
  const updateFontWeightById = useCallback(
    async (
      nodeId: string,
      weight: AllNodeUIProperties['weight']
    ): Promise<CreateStatus> => {
   ...
   */
  const updateStylesById = useCallback(
    async (
      nodeId: string,
      styles: Partial<AllNodeUIProperties>
    ): Promise<CreateStatus> => {
    ...
    async (
      ...
      try {
        ...
        if (failedResults.length > 0) {
          ...
          return {
            ok: false,
            error: errors || 'Failed to batch update styles',
          };
        }

        console.log(
          '✅ Styles batch updated successfully:',
          updates.length,
          'nodes'
        );
        ...
        return {
          ok: false,
          error: `Failed to batch update styles: ${error instanceof Error ? error.message : String(error)}`,
        };
      }
    },
    [updateStyles]
   ...
   */
  const batchUpdateStylesById = useCallback(
    async (
   ...
   */
  const applyStylePresetById = useCallback(
    async (nodeId: string, presetName: string): Promise<CreateStatus> => {
      const node = getNode(nodeId);
      if (!node) {
        return { ok: false, error: `Node ${nodeId} not found` };
      }
