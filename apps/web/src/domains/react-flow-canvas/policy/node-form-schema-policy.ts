 ...
 */
export function generateDefaultFormSchemaByType(
  nodeType: BlockType
): FormSchema {
  switch (nodeType) {
    case 'shape':
 ...
 */
export function getDefaultValueByFieldType(
...
export function extractUserDefinedSchema(
  ...
  const userDefinedFields = mergedFields.filter(
    field => !defaultFieldIds.has(field.id)
  );
...
export function extractComponentInstanceUserSchema(
  ...
  const defaultFieldIds = new Set(
    defaultSchema?.fields?.map(field => field.id) || []
  );
  const definitionFieldIds = new Set(
    definitionSchema?.fields?.map(field => field.id) || []
  );

  // 기본 스키마와 정의 스키마에 없는 필드들만 추출 (인스턴스별 사용자 정의 필드)
  const instanceUserFields = mergedFields.filter(
    field => !defaultFieldIds.has(field.id) && !definitionFieldIds.has(field.id)
  );
