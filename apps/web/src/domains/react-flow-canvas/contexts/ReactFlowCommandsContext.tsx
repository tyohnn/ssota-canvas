...
export function useReactFlowCommandsContext(): ReturnType<
  typeof useReactFlowCommands
> {
  const ctx = useContext(ReactFlowCommandsContext);
  if (!ctx)
    throw new Error(
      'useReactFlowCommandsContext must be used within a ReactFlowCommandsProvider'
    );
  return ctx;
}

export function ReactFlowCommandsProvider({
  children,
}: {
