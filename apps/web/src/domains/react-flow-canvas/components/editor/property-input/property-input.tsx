...
export function PropertyInput({
  ...
  const renderFieldInput = () => {
    switch (field.type) {
      ...
      case 'select':
        return (
          <SelectProperty field={field} data={data as string} node={node} />
        );
      case 'status':
        return (
          <StatusProperty field={field} value={data as string} node={node} />
        );
      case 'checkbox':
        return (
          <CheckboxProperty field={field} data={data as boolean} node={node} />
        );
      case 'url':
        return (
          <UrlProperty
      ...
      case 'shape':
        return (
          <ShapeProperty field={field} value={data as string} node={node} />
        );
      case 'date':
        return (
          <DateProperty
      ...
      case 'file':
        return (
          <FileProperty field={field} data={data as FileItem[]} node={node} />
        );
      case 'email':
        return (
          <EmailProperty
      ...
      case 'hidden':
        return <HiddenProperty field={field} data={data} node={node} />;
      default:
        return (
          <TextProperty
            field={field}
  ...
  return (
    <div
      className={`grid grid-cols-[auto_1fr_auto] items-center gap-2 py-1 px-3 rounded-md${isOverridden ? 'border-orange-200 bg-orange-50' : ''}`}
    >
      {isPredefined ? (
