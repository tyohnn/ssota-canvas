  ...
  const updateField = useCallback(async (
    ...
    if (nodeType === 'component-instance') {
      if (fieldType === 'formData') {
        const currentFormData = node.data.formData as ComponentInstanceMetadata["formData"] || {};
        ...
        const result = await componentCommands.updateComponentInstanceFormData(
            node,
          updatedFormData,
          fieldId
        );
        ...
        if (!result.ok) {
          console.error("Failed to update component instance form data:", result.error);
        }
      } else if (fieldType === 'nodeUI') {
        const currentNodeUI = node.data.nodeUI as ComponentInstanceMetadata["nodeUI"] || {};
        const key = path[1] as string;
        const updatedNodeUI = {
          ...currentNodeUI,
          [key]: value
        ...
        if (!result.ok) {
          console.error("Failed to update component instance nodeUI:", result.error);
        }
      }
    }
    // 일반 노드 및 컴포넌트 정의 처리
    else if (nodeType === 'regular-block' || nodeType === 'component-definition') {
      if (fieldType === 'formData') {
        const currentFormData = node.data.formData as DefaultMetadata["formData"] || {};
        const key = path[1] as string;
        const updatedFormData = {
          ...currentFormData,
          [key]: value
        ...
        if (!result.ok) {
          console.error("Failed to update regular node form data:", result.error);
        }
      } else if (fieldType === 'nodeUI') {
        ...
        if (!result.ok) {
          console.error("Failed to update regular node nodeUI:", result.error);
        }
      }
    }
  ...
  const resetField = useCallback(
    async (node: Node, field: SchemaField) => {
      ...
      if (nodeType === 'component-instance') {
        ...
        if (!result.ok) {
          console.error("Failed to reset component instance field:", result.error);
        }
      } else {
