// apps/web/src/domains/workspace-management/frontend/components/page-tree/components/page-tree-item.tsx
'use client';
...
import type { PageTreeItem } from '../core/types';
import type { PageTreeState } from '../core/use-page-tree';
...
export function PageTreeItemRenderer({
  ...
  const { createPage, selectPage, deletePage, duplicatePage } = pageTreeState;
  const [isHovered, setIsHovered] = useState(false);
  const [isChevronHovered, setIsChevronHovered] = useState(false);
  const [isMenuOpen, setIsMenuOpen] = useState(false);

  const isActive = selectedPageId === page?.pageId;
  ...
  const handleDuplicate = useCallback(async () => {
    if (!page.pageId) return;
    await duplicatePage(page.pageId);
