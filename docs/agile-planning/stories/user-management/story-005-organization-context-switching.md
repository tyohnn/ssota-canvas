...
### Backend Domain
- [x] `UserAggregate`에 `selectOrganization`, `setOrganizationContext` Command Handler 구현
- [x] `OrganizationSelectedByUserEvent`, `OrganizationContextSetEvent` 도메인 이벤트 정의
- [x] `UserManagementService`에 조직 전환 메서드 추가
- [x] 조직 멤버십 권한 검증 로직 구현

### Database & Repository
- [x] `user_sessions` 테이블에 `current_organization_id` 필드 추가
- [x] `user_organizations_view` Read Model 뷰 생성
- [x] `UserRepository`, `MembershipRepository`에 조직 전환 관련 메서드 구현

### API & Server Action
- [x] `selectOrganizationAction`, `setOrganizationContextAction` Server Action 구현
- [x] 조직 전환 권한 검증 로직
- [x] 에러 처리 및 사용자 피드백

### Frontend
- [x] **사이드바 조직 선택기**: `OrganizationSelector` 컴포넌트 구현
  - `Select`, `SelectContent`, `SelectItem`, `SelectTrigger`, `SelectValue` UI 컴포넌트 활용
  - 조직 목록 드롭다운으로 표시
  - 현재 선택된 조직 하이라이트 표시
  - 조직 전환 시 즉시 UI 반영
- [x] **조직 목록 표시**: 사용자 소유/소속 조직 목록 UI
  - 조직명, 역할(Owner/Admin/Member) 표시
  - 기본 조직 표시 (Badge 컴포넌트 활용)
  - 조직 멤버 수 표시
- [x] **현재 조직 컨텍스트**: 헤더/사이드바에 현재 조직 표시
  - 사이드바 상단에 현재 조직명 표시
  - 조직 전환 시 헤더 정보 업데이트
- [x] **조직 전환 로딩**: 전환 시 스피너 및 상태 관리
  - `Loader2` 아이콘으로 로딩 상태 표시
  - `useTransition` Hook으로 pending 상태 관리
  - 전환 중 조직 선택기 비활성화
- [x] **React Context 연동**: `UserManagementContext`를 통한 상태 관리
  - `useUserManagement()` Hook으로 조직 데이터 접근
  - `selectOrganization` 액션으로 조직 전환 처리
  - Context 상태와 UI 동기화
- [x] **낙관적 업데이트**: 즉시 UI 반영 후 서버 검증
