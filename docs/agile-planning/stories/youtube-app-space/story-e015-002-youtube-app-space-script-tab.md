...
- [ ] Script 탭 컴포넌트 구현
  ...
  - 스크립트 편집 UI
  - 스크립트 추출 기능
- [ ] Script 탭을 YouTube 블록 탭에 연결

---

### 도메인 간 통합
- [ ] **YouTube App Space → Block Management 연동**
  - 블록 properties에 `youtubeId` 저장
  - YouTube 데이터 조회 시 블록 소유권 확인
- [ ] **Block Management → YouTube App Space 연동**
...
### Testing & Quality
- [ ] Unit Tests (Repository, Service Functions)
- [ ] Integration Tests (Server Actions, 권한 검증)
- [ ] E2E Tests (YouTube 생성 → 스크립트 추출 → Script 탭 표시)
- [ ] 데이터 중복 제거 테스트 (같은 영상 100개 블록 사용 시 1개만 저장)
- [ ] RLS 정책 테스트 (권한 없는 사용자 접근 차단)
- [ ] JSONB 쿼리 성능 테스트
