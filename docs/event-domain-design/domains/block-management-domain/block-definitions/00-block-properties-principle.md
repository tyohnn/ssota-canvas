...
**Properties = 사용자가 직접 제어하는 블록의 설정값 (2-3개 권장)**
...
export interface GithubBranchBlockProperties {
  repository: string;   // 사용자 입력
  branchName: string;   // 사용자 입력
}

// ❌ Properties에 포함하지 않음 (컴포넌트 내부 state)
// - lastCommit (자동 fetch)
// - commitCount (자동 fetch)
// - status (자동 fetch)
// - updatedAt (자동 fetch)
```
...
export interface VercelDeploymentBlockProperties {
  projectName: string;     // 사용자 입력
  deploymentUrl: string;   // 사용자 입력
}

// ❌ Properties에 포함하지 않음 (자동 fetch)
// - status (자동 fetch - Vercel API)
// - branch (자동 fetch)
// - commitMessage (자동 fetch)
// - deployedAt (자동 fetch)
// - buildTime (자동 fetch)
```
...
**역할:** 블록의 설정과 메타 정보를 한 곳에 모음

**포함 파일:**
1. **`{blockname}-editor-panel-schema.ts`**
   - Editor Panel UI 정의
   - 속성 그룹, 입력 타입, 라벨 등

2. **`{blockname}-block-action-schemas.ts`** (선택)
   - AI Agent 액션 파라미터 검증 (Zod)
   - Runtime 파라미터 체크

3. **`{blockname}-ai-definition.ts`** (선택)
   - AI Agent용 블록 정의
   - 블록 설명, 사용 사례, 액션 목록
...
**역할:** AI Agent 액션 로직과 사용자 Action UI

**포함 파일:**
1. **`{blockname}-block-actions.ts`** (AI Agent용)
   - 순수 함수 `executeAction` export
   - Hook 사용 금지
   - 동적 import로 로드: `block-type/{blocktype}/action-items/{blocktype}-block-actions.ts`

2. **`{action-name}-action/`** (사용자 UI용)
   - components/ + core/ 패턴
   - 사용자가 직접 클릭하여 사용하는 Action UI
...
type BlockPropertiesMap = {
  ...
  your_new_block: YourBlockProperties; // ← 추가 쉬움
};
