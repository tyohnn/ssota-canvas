...
2. 기본 워크스페이스가 생성됨 (Default Workspace Created)
3. 개인 워크스페이스가 생성됨 (Personal Workspace Created)

---

**Policy**: 
- "Whenever 조직 생성이 완료됨, then always 조직 목록을 갱신하고 새 조직으로 컨텍스트 전환하기"
- "Whenever 새 조직이 생성됨, then always 기본 워크스페이스와 소유자 개인 워크스페이스를 자동 생성하기"
...
2. 개인 워크스페이스가 생성됨 (Personal Workspace Created) - 새 멤버용
3. 초대 거절됨 (Invitation Rejected)
4. 알림 읽혀짐 (Notification Read)
...
2. **워크스페이스 자동 생성**: 조직 생성 시 자동으로 2개의 워크스페이스 생성
   - Default Workspace: 조직 전체 협업 공간 (모든 멤버 접근 가능)
   - 개인 워크스페이스: 소유자 전용 공간 (소유자만 접근, 초대 불가)
3. **멤버 초대 승낙 시 자동 생성**: 초대 승낙 시 해당 멤버의 개인 워크스페이스 자동 생성
4. **조직 이름 고유성**: 조직 이름은 플랫폼 내에서 고유해야 함
5. **3단계 역할 시스템**: 소유자 > 관리자 > 멤버 권한 체계
6. **소유권 이전**: 소유자 역할은 이전을 통해서만 변경 가능

### 워크스페이스 종류 및 권한
7. **Default Workspace**: 조직 생성 시 자동 생성, 모든 멤버 접근 가능, 멤버 초대 가능
8. **일반 워크스페이스**: 조직 멤버가 생성, 선택적으로 멤버 초대 가능
9. **개인 워크스페이스**: 각 멤버 전용, 소유자만 접근, 다른 멤버 초대 불가

### 초대 및 멤버 관리 관련 (핵심)
10. **권한 기반 초대**: 소유자와 관리자만 멤버 초대 가능
11. **이메일 검증**: 초대 수락 시 이메일 주소 검증 필수
12. **30일 초대 유효기간**: 초대 링크 30일 후 자동 만료
13. **개인 워크스페이스 자동 생성**: 초대 승낙 시 해당 멤버의 개인 워크스페이스 자동 생성

### 삭제 및 보안
14. **소프트 삭제**: 30일 유예 기간 제공
15. **계층적 삭제**: 조직 삭제 시 하위 요소 함께 처리
16. **Danger Zone**: 조직 삭제는 이름 확인 + 소유자 권한 필수
