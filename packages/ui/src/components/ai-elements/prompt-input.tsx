  ...
  const handleSubmit: FormEventHandler<HTMLFormElement> = event => {
    ...
    Promise.all(
      files.map(async ({ id, ...item }) => {
        void id; // id is intentionally unused, keeping it for destructuring
      ...
      try {
        const result = onSubmit(
          { text, files: convertedFiles } as PromptInputMessage,
          event
        );
...
export const PromptInputTextarea = ({
  ...
  return (
    <InputGroupTextarea
      className={cn(
        'field-sizing-content max-h-48 min-h-8 bg-background',
        className
      )}
      name="message"
