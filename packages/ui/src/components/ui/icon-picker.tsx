...
const useIconsData = () => {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      setIsLoading(true);
      // @ts-expect-error - importing TS file from monorepo workspace under NodeNext
      const mod = await import("./icons-data.ts");
      if (!mounted) return;
      setIcons((mod.iconsData || []) as IconData[]);
      setIsLoading(false);
    };
    load();
    return () => {
    ...
    const categoryIndices = useMemo(() => {
      ...
      virtualItems.forEach((item, index) => {
        if (item.type === "category") {
          const cat = categorizedIcons[item.categoryIndex];
          if (cat) indices[cat.name] = index;
        }
      });
