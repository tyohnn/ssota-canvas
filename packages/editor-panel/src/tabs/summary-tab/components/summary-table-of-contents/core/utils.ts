...
export function extractHeadingsFromTiptapJSON(json: JSONContent | null | undefined): SummaryTOCItem[] {
      ...
      if (level >= 1 && level <= 3) {
        ...
        if (text.trim()) {
          const id = `heading-${index}-${text
            ...
            .replace(/[^a-z0-9]+/g, '-')
            .replace(/^-+|-+$/g, '')}`;
          headings.push({ id, level: level as 1 | 2 | 3, text: text.trim(), elementId: id });
          index++;
        }
      }
    }
